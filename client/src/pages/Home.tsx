...
function BookingPanel({ onClose }: { onClose?: () => void }) {
  ...
  const [success, setSuccess] = useState<{ bookingCode: string; serviceName: string } | null>(null);
  ...
  const copy = language === "ar" ? {
    eyebrow: "احجز مكالمة استكشافية", title: "اختر الموعد المناسب لك.", service: "ما الخدمة التي تحتاجها؟", choose: "اختر خدمة", date: "التاريخ المفضل", available: "المواعيد المتاحة", session: "دقيقة للجلسة", checking: "جارٍ التحقق من المواعيد…", empty: "لا توجد مواعيد متاحة في هذا اليوم.", name: "الاسم بالكامل", phone: "رقم WhatsApp مع كود الدولة", email: "البريد الإلكتروني (اختياري)", notes: "حدثنا عن نشاطك التجاري (اختياري)", confirmation: "سيتم إرسال التأكيد إلى WhatsApp الخاص بك.", confirm: "تأكيد الحجز", confirming: "جارٍ التأكيد…", successTitle: "تم تأكيد حجزك بنجاح!", successText: "شكرًا لك. أرسلنا تفاصيل الحجز إلى WhatsApp وسنتواصل معك قريبًا.", code: "رقم الحجز", done: "تم", newBooking: "حجز موعد آخر", close: "إغلاق"
  } : {
    eyebrow: "BOOK A DISCOVERY CALL", title: "Choose a time that works.", service: "What do you need?", choose: "Select a service", date: "Preferred date", available: "Available times", session: "minute session", checking: "Checking live availability…", empty: "No times left on this day. Try another date.", name: "Full name", phone: "WhatsApp number (with country code)", email: "Email (optional)", notes: "Tell us a little about your business (optional)", confirmation: "Confirmation is sent to your WhatsApp.", confirm: "Confirm booking", confirming: "Confirming…", successTitle: "Your booking is confirmed!", successText: "Thanks for booking with Take More. We sent the details to WhatsApp and will be in touch shortly.", code: "Booking code", done: "Done", newBooking: "Book another time", close: "Close"
  };
  const createBooking = trpc.booking.create.useMutation({
    onSuccess: result => { setSuccess({ bookingCode: result.bookingCode, serviceName: result.serviceName }); setSlot(""); setDate(""); setForm({ customerName: "", customerPhone: "", customerEmail: "", notes: "" }); },
